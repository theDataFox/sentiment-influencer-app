...
def _quote(text):
    return urllib.parse.quote(text, '-._~')
...
class OAuthHandler(urllib.request.BaseHandler):
    ...
    def get_signature(self, method, uri, query):
        ...
        # key = bytes(key, 'utf-8')
        signature = hmac.new(b'{key}', signature_base.encode('utf-8'), hashlib.sha1)
    ...
    def http_request(self, req):
        ...
        query = [(k_v[0], urllib.parse.quote(k_v[1])) for k_v in urllib.parse.parse_qsl(query)]
        ...
        if hasattr(req, 'oauth_params'):
            oauth_params += list(req.oauth_params.items())
        ...
        req = Request(req.get_full_url(), data=req.data(), headers=req.headers,
                      origin_req_host=req.get_origin_req_host(), unverifiable=req.is_unverifiable(), method=method)
...
'>>> https://apps.twitter.com/\n''')
    ...
    try:
        ...
        opener = urllib.request.build_opener(OAuthHandler(consumer))
        resp = opener.open(Request('https://api.twitter.com/oauth/request_token'))
        rtoken = urllib.parse.parse_qs(resp.read())
...
'>>> https://api.twitter.com/oauth/authorize?oauth_token=%s\n''' % rtoken.key)
        ...
        opener = urllib.request.build_opener(OAuthHandler(consumer, rtoken))
        resp = opener.open( \
            Request('https://api.twitter.com/oauth/access_token', \
                    oauth_params={'oauth_verifier': verifier}))
        atoken = urllib.parse.parse_qs(resp.read())
    ...
    opener = urllib.request.build_opener(OAuthHandler(consumer, atoken))
...
def resolve(args):
    for sn in args.screen_name:
        try:
            ...
            conn = urllib.request.urlopen(url % sn)
...
class CursorError(urllib.error.HTTPError):
    def __init__(self, code, cursor=None, res=None, headers=None):
        urllib.error.HTTPError.__init__(self, None, code, None, None, None)
        self.cursor = cursor
        self.res = res
...
def _ids(relation, param, c=None, init=None):
    ...
    while True:
        try:
            ...
            conn = urllib.request.urlopen(url % (relation, param, cursor))
        except urllib.error.HTTPError as e:
...
def _members(param, c=None, init=None):
    ...
    while True:
        try:
            ...
            conn = urllib.request.urlopen(url % (param, cursor))
        except urllib.error.HTTPError as e:
        ...
        while True:
            try:
                if args.l:
                    ...
                    url = 'https://api.twitter.com/1.1/users/show.json?screen_name=%s'
                    conn = urllib.request.urlopen(url % args.screen_name)
                    data = json.loads(conn.read())
                    conn.close()
    ...
    while len(bag) > 0:
        ...
        if len(bag) > 100:
            while len(next_items) < 100:
                next_items = [bag.pop() for _ in range(100)]
        else:
            next_items = bag
            bag = []
        ...
        try:
            ...
            conn = urllib.request.urlopen(url % items_str)
...
def edgelist(args):
    ...
    for sn in args.screen_name:
        for item in csv.reader(_skip_hash(open(DAT_PATH + '/' + sn + TT_EXT))):
            if item[0] not in dat:
                ...
                # col 10: location
                # col 11: handle arg
    id0 = {user_id: val[0] for user_id, val in dat.items() if val[1] in args.screen_name}
    e = open(GML_PATH + '/' + '_'.join(args.screen_name) + EDG_EXT, 'w')
    e.write('graph [\n  directed 1\n')
    j = 0
    for user_id, val in dat.items():
    ...
    for id1, val in dat.items():

        if id1 in list(id0.keys()):
            continue
        if not args.missing and not os.path.isfile(FDAT_DIR + '/' + str(id1) + FDAT_EXT):
            continue
        if args.ego:
            for idzero in list(id0.values()):
                d = _days(val[7])
                j += 1
                e.write('''  edge [
    id %s            
...
''' % (j, idzero, val[0], float(val[6]) / d if float(d) > 0 else 0))
        ...
        if os.path.isfile(filename):
            ...
            for line in fdat:
                id2 = line.strip()
                if id2 in dat:
                    if not args.ego and id2 in list(id0.keys()):
                        continue
...
def _destroy(screen_name):
    ...
    while len(dat) > 0:
        ...
        try:
            ...
            data = {'id': item}
            conn = urllib.request.urlopen(url, urllib.parse.urlencode(data))
        except urllib.error.HTTPError as e:
            if e.code in SKIP_CODES:
                sys.stdout.write('HTTPError %s with %s. Skipping...\n' % (e.code, item))
                continue
    ...
    if args.purge:
        ...
        while True:
            try:
                ...
                if max_id:
                    url += '&max_id=%i' % max_id
                conn = urllib.request.urlopen(url % (200, args.screen_name))
            except urllib.error.HTTPError as e:
...
class QuoteAction(argparse.Action):
    def __call__(self, parse, namespace, values, option_string=None):
        if isinstance(values, list):
            setattr(namespace, self.dest, [urllib.parse.quote(v) for v in values])
        else:
            setattr(namespace, self.dest, urllib.parse.quote(values))
